...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const alternates = buildAlternates(locale, "/guide/srt-vs-vtt");

  const title = "SRT vs VTT: Complete Comparison Guide for Developers | YTVidHub";
  const description = "Comprehensive technical comparison of SRT and WebVTT subtitle formats. Learn which format is best for AI training, web development, and bulk subtitle extraction.";
  ...
  return {
    ...
    openGraph: {
      ...
      images: [{ url: "/image/srt-vs-vtt-comparison-og.webp", width: 1200, height: 630, alt: "SRT vs VTT Subtitle Format Comparison Guide" }],
    },
    twitter: {
...
export default async function SrtVsVttLayout({ children, params }: Props) {
  const { locale } = await params;
