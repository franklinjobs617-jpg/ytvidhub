...
export async function POST(request: NextRequest) {
    try {
        ...
        if (currentCredits < 2) {
            return NextResponse.json(
                { error: 'Insufficient credits. AI Summary requires 2 credits.' },
                { status: 402 },
            )
        }
        ...
        if (!backendResponse.ok) {
            return NextResponse.json(
                { error: await parseErrorMessage(backendResponse, 'Failed to generate summary') },
