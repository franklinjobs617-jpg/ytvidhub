...
export function MobileNavigation({
  ...
  return (
    ...
    >
      <div className="pointer-events-auto mx-auto grid max-w-[360px] grid-cols-2 gap-1 rounded-2xl border border-slate-200/80 bg-white/95 p-1.5 shadow-[0_10px_36px_-20px_rgba(15,23,42,0.5)] backdrop-blur">
        {TABS.map((tab) => {
          ...
          return (
            ...
            >
              {showPulse ? (
                <Loader2
                  size={16}
                  className={`animate-spin ${isActive ? "text-white" : "text-blue-600"}`}
                />
              ) : (
                <tab.icon
                  size={16}
              ...
              <span
                className={`text-xs font-semibold tracking-tight ${isActive ? "text-white" : "text-slate-700"}`}
              >
                {tab.label}
              </span>
