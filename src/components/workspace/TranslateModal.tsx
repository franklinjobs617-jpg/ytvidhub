...
export function TranslateModal({ isOpen, onClose, videoUrl, videoTitle }: TranslateModalProps) {
  ...
  useEffect(() => {
    if (isOpen && videoUrl) {
      ...
      .then(async (blob) => {
        ...
        if (parsed.length === 0) {
          setError(`Failed to parse subtitles. First 500 chars: ${text.substring(0, 500)}`);
        } else {
          setEntries(parsed);
          // 保存到缓存
          try {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] flex flex-col">
        <div className="p-5 bg-gradient-to-r from-purple-600 to-indigo-600 flex items-center justify-between rounded-t-2xl">
          <div className="flex items-center gap-2">
            <Languages className="w-6 h-6 text-white" />
            <h2 className="text-xl font-bold text-white">Bilingual Subtitles</h2>
          </div>
        ...
        <div className="p-5 border-b flex items-center gap-3">
          ...
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700 disabled:bg-gray-300 text-sm font-medium"
          >
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-3">
          {isLoading ? (
            ...
            <div className="text-center text-gray-500 py-8">No subtitles available</div>
          ) : (
            entries.map((entry, i) => (
              <div key={i} className="border-b pb-3">
                ...
                {translations[i] && <div className="text-sm text-purple-600 mt-1">{translations[i]}</div>}
              </div>
            ))
          )}
        </div>
